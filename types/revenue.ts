...
export type ServiceRevenueSummary = {
  service: string;
  collected: number;
  invoiceCount: number;
  averageInvoice: number;
};
export type DivisionRevenueSummary = {
...
export type MonthlyPerformance = {
  collected: MonthlyMetric;
  invoicesIssued: MonthlyMetric;
  paymentsReceived: MonthlyMetric;
  outstanding: MonthlyMetric;
};
export type CompletedWorkValue = {
  total: number;
  average: number;
  count: number;
};
...
export type RevenueReport = {
  ...
  overTime: RevenueDataPoint[];
  byClient: ClientRevenueSummary[];
  byService: ServiceRevenueSummary[];
  byDivision: DivisionRevenueSummary[];
  byMethod: PaymentMethodSummary[];
  outstandingInvoices: InvoiceWithRelations[];
  pastDue: PastDueSummary;
  recentPayments: RevenuePaymentRecord[];
  monthly: MonthlyPerformance;
  completedWork: CompletedWorkValue;
  topCustomers: ClientRevenueSummary[];
};
