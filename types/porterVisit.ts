...
export type CreatePorterVisitInput = { plan_id: string; scheduled_date: string; scheduled_start_time: string; assigned_crew_id: string | null; visit_notes: string | null };
export type PorterVisitEdit = Pick<PorterVisit, "scheduled_date" | "scheduled_start_time" | "assigned_crew_id" | "visit_notes">;
...
export function validateVisitMutation(visit: PorterVisitWithAreas, mutation: PorterVisitMutation, management: boolean) {
  ...
  if (mutation.action === "edit") { validateVisitDate(mutation.data.scheduled_date); validateVisitTime(mutation.data.scheduled_start_time ?? ""); }
