...
export type PorterVisit = {
  ...
  assigned_manager_employee_id: string | null; assigned_worker_employee_id: string | null; assigned_worker_crew_id: string | null;
  plan_name: string; property_label: string; scheduled_date: string; scheduled_start_time: string | null;
  notification_revision: number; status: PorterVisitStatus;
...
export type PorterVisitWithAreas = PorterVisit & { crew_name: string | null; worker_name: string | null; manager_name: string | null; areas: PorterVisitArea[]; photos?: PorterPhoto[]; issues?: PorterIssue[] };
export type CreatePorterVisitInput = { plan_id: string; scheduled_date: string; scheduled_start_time: string; assigned_worker_employee_id: string | null; assigned_worker_crew_id: string | null; assigned_manager_employee_id: string | null; visit_notes: string | null };
export type PorterVisitEdit = Pick<PorterVisit, "scheduled_date" | "scheduled_start_time" | "assigned_worker_employee_id" | "assigned_worker_crew_id" | "assigned_manager_employee_id" | "visit_notes">;
...
export function validateVisitTime(time: string) {
  if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(time.slice(0, 5))) throw new Error("Choose a valid scheduled start time.");
}
export function validatePorterAssignment(input: Pick<CreatePorterVisitInput, "assigned_worker_employee_id" | "assigned_worker_crew_id">) {
  if (input.assigned_worker_employee_id && input.assigned_worker_crew_id) throw new Error("Choose either an individual Porter Tech or a Porter Crew, not both.");
}
export function validateVisitMutation(visit: PorterVisitWithAreas, mutation: PorterVisitMutation, management: boolean) {
  ...
  if (mutation.action === "edit") { validateVisitDate(mutation.data.scheduled_date); validateVisitTime(mutation.data.scheduled_start_time ?? ""); validatePorterAssignment(mutation.data); }
