...
export type Employee={id:string;employee_number:string;first_name:string;last_name:string;preferred_name:string|null;email:string|null;phone:string|null;department:EmployeeDepartment;job_title:string|null;employment_status:EmploymentStatus;employment_type:EmploymentType|null;hourly_rate:number;commission_rate:number;hire_date:string|null;notes:string|null;created_at:string;updated_at:string;archived_at:string|null};
export type EmployeeInput=Omit<Employee,"id"|"employee_number"|"created_at"|"updated_at"|"archived_at">;
export type EmployeeUpdate=Partial<EmployeeInput> & {archived_at?:string|null};
export function employeeName(e:Pick<Employee,"first_name"|"last_name"|"preferred_name">){return e.preferred_name?.trim()||`${e.first_name} ${e.last_name}`.trim()}
