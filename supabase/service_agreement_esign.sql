-- StudioScrubz OS: public Service Agreement review and typed e-signature.
...
alter table public.service_agreements
  ...
  add column if not exists client_signed_snapshot jsonb,
  add column if not exists client_consent_text text,
  add column if not exists client_consent_at timestamptz;

create unique index if not exists service_agreements_client_access_token_key
  on public.service_agreements(client_access_token)
  where client_access_token is not null;

create index if not exists service_agreements_client_access_token_expires_idx
  on public.service_agreements(client_access_token_expires_at)
  where client_access_token is not null;
...
begin
  if old.client_signed_at is not null then
    ...
    if new.client_id is distinct from old.client_id
      ...
      or new.custom_interval_days is distinct from old.custom_interval_days
...
drop trigger if exists prevent_signed_agreement_material_changes on public.service_agreements;
create trigger prevent_signed_agreement_material_changes
before update on public.service_agreements
for each row execute function public.prevent_signed_agreement_material_changes();
...
begin
  ...
  select case
    when a.client_signed_snapshot is not null then
...
-- Remove the prior three-argument overload so explicit consent cannot be bypassed.
drop function if exists public.accept_service_agreement_by_token(text, text, text);
...
begin
  ...
  if p_consent is distinct from true then
    raise exception 'Explicit consent is required to sign this Service Agreement.';
  end if;
  if length(v_name) < 2 or length(v_name) > 150 then
    raise exception 'Enter a valid full legal name.';
  end if;
  if v_signature <> ('/s/ ' || v_name) then
    raise exception 'The typed signature is invalid.';
  end if;
...
-- No table privilege or RLS policy is granted to anon. Anonymous clients can only
-- access the explicit client-facing snapshot returned by these token RPCs.
