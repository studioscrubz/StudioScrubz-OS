...
- Live verification on 2026-09-19 found `invoice_job_photos` absent from every publication even though the UI subscribes to it through `OPERATIONAL_TABLES`. Phase 2D preserves the verified database contract and does not guess at publication membership; this Realtime mismatch remains an explicit follow-up.
