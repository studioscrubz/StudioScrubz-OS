...
const input = { plan_id: "plan", scheduled_date: "2026-09-09", scheduled_start_time: "09:30", assigned_worker_employee_id: null, assigned_worker_crew_id: "crew", assigned_manager_employee_id: null, visit_notes: null };
...
test("six-role read/create permissions and no broad field permissions", async () => {
  for (const role of ["Master Admin", "Administrator", "Manager", "Sales", "Crew Lead", "Scrub Technician"]) {
    ...
    const calls = [];
    const service = api(role, async (name, args) => { calls.push({ name, args }); return { data: name === "create_porter_visit_v3" ? "visit" : [], error: null }; });
    ...
    assert.equal(calls.some(call => call.name === "create_porter_visit_v3"), manager);
...
test("creation only sends plan, schedule, structured assignments and notes", async () => {
  let payload;
  ...
  assert.deepEqual(Object.keys(payload).sort(), ["p_assigned_manager_employee_id", "p_assigned_worker_crew_id", "p_assigned_worker_employee_id", "p_notes", "p_plan_id", "p_scheduled_date", "p_scheduled_start_time"]);
  ...
  assert.throws(() => models.validatePorterAssignment({ assigned_worker_employee_id: "tech", assigned_worker_crew_id: "crew" }), /either/);
});
