...
test("job event boundaries request push only after successful authoritative results", () => {
  ...
  assert.match(source, /update_operational_job[\s\S]*const job=operationalJob\(data\);[\s\S]*requestJobAttentionPush\(job\)/);
  ...
  assert.match(source, /Ready to Schedule[\s\S]*Scheduled[\s\S]*!job\.assigned_crew_id && !job\.assigned_employee_id/);
...
test("communication failure, open time, and restore boundaries are wired", () => {
  ...
  assert.match(startOperationalJob, /if \(error\) throw[\s\S]*requestImmediateAttentionPush/);
