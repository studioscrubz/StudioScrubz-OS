...
export async function getEmployees(): Promise<Employee[]> {
  ...
  if (
    profile &&
    ["Sales", "Crew Lead", "Scrub Technician"].includes(profile.role)
  ) {
    const { data, error } = await getSupabaseClient()
      .from("employee_directory_company_safe")
      .select("*")
      .order("last_name");
...
export async function getEmployeesByDepartment(
  ...
  return (await getEmployees()).filter((employee) =>
...
export async function createEmployee(
  ...
  if (isMasterAdmin(await getCurrentProfile())) {
    ...
    throw new Error("A unique employee number could not be generated.");
  }

  const { data, error } = await getSupabaseClient().rpc(
    "admin_operational_create_employee",
    {
      p_employee_number: number(),
      ...operationalArgs(input),
    },
  );
...
export async function updateEmployee(
  ...
  const { data, error } = await getSupabaseClient().rpc(
    "admin_operational_update_employee",
    {
      p_employee_id: id,
      ...operationalArgs(merged),
      p_archive: false,
    },
  );
