...
import type {AuthorizedVehicle,Vehicle,VehicleInput,VehicleUpdate,VehicleWithRelations} from "@/types/vehicle";
...
export async function getAuthorizedVehicles():Promise<AuthorizedVehicle[]>{const{data,error}=await getSupabaseClient().from("authorized_vehicles_safe").select("*").order("vehicle_number");if(error)throw error;return data}
