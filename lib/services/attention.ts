...
export async function getAttentionItems(view: AttentionView = "Active"): Promise<AttentionItem[]> {
  ...
  for (const job of jobs) {
    ...
    if (canManageCommunications && job.client_id && job.scheduled_date && withinReminderWindow(job.scheduled_date, job.start_time, clock.now) && !["Completed", "Cancelled", "Archived"].includes(job.status) && !job.archived_at && !reminderWasSent(communications, "Job", job.id, job.scheduled_date)) {
      ...
      items.push(item(`service-reminder:${job.id}:${job.scheduled_date}`, "Service Reminder Due", "Info", "Communications", "Service reminder due", `${label} · ${job.service_name || "Service"} — ${job.client_name || "Deleted Client"}`, "Job", job.id, job.client_id, label, null, job.scheduled_date, job.created_at, `/jobs?jobId=${job.id}`, "Open Job", composer({ clientId: job.client_id, propertyId: job.property_id, jobId: job.id, communicationType: "Service Reminder", client: job.client, fallbackName: job.client_name, email: job.client?.email, phone: job.client?.phone, subject: `Reminder: ${job.service_name || "StudioScrubz Service"} — ${date}`, message: `Hello ${greeting(job.client, job.client_name)},\n\nThis is a reminder that your StudioScrubz ${job.service_name || "service"} is scheduled for ${date}${time ? ` at ${time}` : ""}${location ? ` at ${location}` : ""}.\n\nIf you need to make any changes, please contact us.\n\nThank you,\nStudioScrubz`, sourceType: "Job", sourceId: job.id, metadata: { source: "Job", source_id: job.id, scheduled_date: job.scheduled_date, service_name: job.service_name || "Service" } })));
    }
  }
  ...
  for (const invoice of invoices.filter((row) => !row.archived_at && row.balance_due > 0 && !["Paid", "Cancelled", "Archived"].includes(row.status))) {
    const reminderContext = invoice.due_date ? composer({ clientId: invoice.client_id, propertyId: invoice.property_id, invoiceId: invoice.id, communicationType: "Payment Reminder", client: invoice.client, fallbackName: invoice.client_name, email: invoice.customer_email ?? invoice.client?.email, phone: invoice.customer_phone ?? invoice.client?.phone, subject: `Reminder: StudioScrubz Invoice ${invoice.invoice_number}`, message: `Hello ${greeting(invoice.client, invoice.client_name)},\n\nThis is ${invoice.due_date < today ? "a reminder" : "a friendly reminder"} that StudioScrubz Invoice ${invoice.invoice_number} has an outstanding balance of ${money(invoice.balance_due)} and ${invoice.due_date < today ? "was" : "is"} due on ${friendlyDate(invoice.due_date)}.${invoice.due_date < today ? "\n\nPlease contact us if you have any questions regarding your invoice." : ""}\n\nThank you,\nStudioScrubz`, sourceType: "Invoice", sourceId: invoice.id, metadata: { invoice_number: invoice.invoice_number, follow_up: true } }) : null;
  ...
  for (const entry of timeEntries.filter((row) => row.status === "Open" && !row.clock_out && !row.archived_at)) items.push(item(`time:${entry.id}:open`, "Open Time Entry", "Attention", "Time", "Time entry is still open", `${entry.time_entry_number} · ${entry.employee ? [entry.employee.first_name, entry.employee.last_name].filter(Boolean).join(" ") : "Deleted Employee"}`, "Time Entry", entry.id, null, entry.time_entry_number, null, entry.work_date, entry.created_at, "/time-clock", "Open Time Clock"));
...
export async function restoreAttentionItem(attentionKey: string): Promise<void> { const profile = await getCurrentProfile(); if (!profile) throw new Error("Authentication is required."); const { error } = await getSupabaseClient().from("attention_item_states").delete().eq("user_id", profile.id).eq("attention_key", attentionKey); if (error) throw new Error(`Attention item could not be restored: ${error.message}`); }
...
function item(id: string, type: AttentionItem["type"], severity: AttentionItem["severity"], category: AttentionItem["category"], title: string, description: string, record_type: string, record_id: string, client_id: string | null, entity_label: string | null, due_date: string | null, scheduled_date: string | null, created_at: string, action_url: string, action_label: string, communication_context: CommunicationComposerContext | null = null): AttentionItem { const dismissible = type === "Upcoming Job" || type === "Service Reminder Due" || type === "Failed Client Communication" || severity === "Info"; return { id, type, severity, category, title, description, record_type, record_id, client_id, entity_label, due_date, scheduled_date, created_at, action_url, action_label, metadata: {}, snoozable: true, dismissible, attention_state: null, communication_context }; }
