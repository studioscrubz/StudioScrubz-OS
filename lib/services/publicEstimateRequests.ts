...
import type { CommercialCalculatorInput, Condition, CustomerInformation, EstimateDivision, EstimateResult, Frequency, ResidentialCalculatorInput } from "@/types/estimate";
...
export type PublicRequest = { division:EstimateDivision; customer:CustomerInformation; service:string; frequency:Frequency; squareFeet:number; bedrooms?:number; bathrooms?:number; occupied?:boolean; pets?:boolean; floors?:number; restrooms?:number; kitchens?:number; stations?:number; units?:number; condition:Condition; addons:string[]; preferredDate?:string; notes?:string; website?:string };
...
export async function submitPublicRequest(input:PublicRequest,result:EstimateResult){
  ...
  const authoritativeResult={...result,submission:{source:"Customer Self-Service",preferredServiceDate:clean(input.preferredDate??""),submittedAt:new Date().toISOString()}};
...
function validate(input:PublicRequest){if(!input||!["Residential","Commercial"].includes(input.division))throw new PublicEstimateRequestError("Choose Residential or Commercial.");const c=input.customer;if(!c?.firstName?.trim()||!c.lastName?.trim()||!c.phone?.trim()||!/^\S+@\S+\.\S+$/.test(c.email??"")||!c.address?.trim()||!c.city?.trim()||!c.state?.trim()||!/^\d{5}(?:-\d{4})?$/.test(c.zip??""))throw new PublicEstimateRequestError("Complete the required contact and service-address fields.");if(input.division==="Commercial"&&!c.companyName?.trim())throw new PublicEstimateRequestError("Company name is required for commercial estimates.");if(!validNumber(input.squareFeet,100,1000000))throw new PublicEstimateRequestError("Enter valid square footage.");const counts=input.division==="Residential"?[input.bedrooms,input.bathrooms]:[input.floors,input.restrooms,input.kitchens,input.stations,input.units];if(counts.some(value=>!validNumber(value??0,0,10000)))throw new PublicEstimateRequestError("Enter valid property details.");if(input.division==="Commercial"&&!validNumber(input.floors??1,1,1000))throw new PublicEstimateRequestError("Enter at least one floor.");if(!["Light","Average","Heavy","Extreme"].includes(input.condition))throw new PublicEstimateRequestError("Choose a property condition.");if(!Array.isArray(input.addons)||input.addons.length>30||input.addons.some(value=>typeof value!=="string"||value.length>200))throw new PublicEstimateRequestError("Invalid add-on selection.");if(typeof input.notes!=="undefined"&&(typeof input.notes!=="string"||input.notes.length>2000))throw new PublicEstimateRequestError("Special requests must be 2,000 characters or less.")}
