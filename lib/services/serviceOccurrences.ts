...
const select = "*, agreement:service_agreements!service_occurrences_agreement_id_fkey(*, client:clients!service_agreements_client_id_fkey(*)), crew:crews!service_occurrences_assigned_crew_id_fkey(*), job:jobs!service_occurrences_job_id_fkey(*)";

export function isDeletedOccurrence(occurrence: Pick<ServiceOccurrence, "notes">) {
...
export async function getOccurrencesForAgreement(id: string) {
  ...
  return rows.filter((row) => row.agreement_id === id);
}
...
export async function reconcileFutureOccurrences(
  ...
  const [agreement, settings, existing] = await Promise.all([
    getAgreementById(agreementId),
    getBusinessSettings(),
    getOccurrencesForAgreement(agreementId),
  ]);

  const currentDate = businessDate(settings.timezone);
  const end = add(currentDate, horizonDays);
  const windowStart =
    agreement.start_date > currentDate
    ...
    agreement.status === "Active"
      ? new Set(
          generateServiceDates(
            ...
            agreement.frequency,
            {
              daysOfWeek: agreement.days_of_week,
              intervalWeeks: agreement.interval_weeks,
              dayOfMonth: agreement.day_of_month,
              secondDayOfMonth: agreement.second_day_of_month,
              thirdDayOfMonth: agreement.third_day_of_month,
              customIntervalDays: agreement.custom_interval_days,
            },
            agreement.end_date,
  ...
  const future = existing.filter(
    (row) => row.scheduled_date >= currentDate
  );

  const byDate = new Map(
    future.map((row) => [row.scheduled_date, row])
  );

  for (const row of future) {
    if (row.job_id) continue;

    const reconciliationCancellation =
      row.status === "Cancelled" &&
      row.notes?.includes(reconciledOccurrenceMarker);

    if (desired.has(row.scheduled_date)) {
      if (row.status === "Scheduled" || reconciliationCancellation) {
        const notes = reconciliationCancellation
          ? removeMarker(row.notes, reconciledOccurrenceMarker)
          : row.notes;
        ...
        const { error } = await db
          ...
          .update({
            status: "Scheduled",
            scheduled_start_time: agreement.default_start_time,
            assigned_crew_id: agreement.assigned_crew_id,
            notes,
          })
          .eq("id", row.id);
...
export async function createJobFromOccurrence(
  ...
  const { data, error } = await getSupabaseClient().rpc(
    "create_job_from_service_occurrence",
    { p_occurrence_id: id }
  );
  ...
  if (
    ["Scheduled", "Ready to Schedule"].includes(job.status) &&
    job.scheduled_date &&
...
export const skipOccurrence = (id: string) =>
...
export async function deleteOccurrence(id: string) {
  ...
  if (data.job_id) {
    throw new Error(
      "This service already has a Job. Cancel the Job or occurrence instead."
    );
  }

  if (data.status !== "Scheduled") {
    throw new Error(
      "Only an upcoming Scheduled service with no operational history can be deleted."
    );
  }

  if (data.scheduled_date < today()) {
    throw new Error(
      "Past or completed service occurrences cannot be deleted."
    );
  }
  ...
  if (linkedJob.data) {
    throw new Error(
      "This service generated a Job and must remain in operational history."
    );
  }
  ...
  if (!tombstone.data) {
    throw new Error(
      "This service changed and is no longer eligible for deletion."
    );
  }
...
async function update(
  id: string,
  input: Partial<ServiceOccurrence>
) {
  const { data, error } = await getSupabaseClient()
    .from("service_occurrences")
    .update(input)
...
function add(value: string, days: number) {
...
function businessDate(timeZone: string | null) {
  const now = new Date();
