...
export async function createWalkthrough(input: WalkthroughInput): Promise<Walkthrough> { if (input.estimate_id) { const existing = await getWalkthroughForEstimate(input.estimate_id); if (existing) throw new WalkthroughDuplicateError(existing); } const payload = withScheduledStatus(input, input.status); const { data, error } = await getSupabaseClient().from("walkthroughs").insert(payload).select().single(); if (error) { if (error.code === "23505") { const existing = input.estimate_id ? await getWalkthroughForEstimate(input.estimate_id) : null; if (existing) throw new WalkthroughDuplicateError(existing); } throw error; } return data; }
export async function updateWalkthrough(id: string, input: WalkthroughUpdate): Promise<Walkthrough> { const current = await getWalkthroughById(id); const merged = withScheduledStatus({ ...current, ...input }, input.status ?? current.status); const payload: WalkthroughUpdate = { ...input, status: merged.status }; const { data, error } = await getSupabaseClient().from("walkthroughs").update(payload).eq("id", id).select().single(); if (error) throw error; return data; }
...
export async function getWalkthroughForEstimate(estimateId: string): Promise<WalkthroughWithRelations | null> { const { data, error } = await getSupabaseClient().from("walkthroughs").select(walkthroughSelect).eq("estimate_id", estimateId).maybeSingle(); if (error) throw error; return data as WalkthroughWithRelations | null; }
export async function getWalkthroughsForEstimates(): Promise<WalkthroughWithRelations[]> { const { data, error } = await getSupabaseClient().from("walkthroughs").select(walkthroughSelect).not("estimate_id", "is", null); if (error) throw error; return data as WalkthroughWithRelations[]; }
