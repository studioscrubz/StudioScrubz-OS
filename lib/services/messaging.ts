...
export const MESSAGING_REALTIME_TABLES = [
  "conversations",
...
export async function getMessagingUsers(): Promise<MessagingUser[]> {
  ...
  if (error) {
    throw new Error(`Messaging users could not be loaded: ${error.message}`);
  }
...
export async function getDirectConversations(
  ...
  if (conversationError) {
    throw new Error(
      `Conversations could not be loaded: ${conversationError.message}`,
    );
  }
  ...
  if (memberError) {
    throw new Error(
      `Conversation members could not be loaded: ${memberError.message}`,
    );
  }

  if (messageError) {
    throw new Error(`Messages could not be loaded: ${messageError.message}`);
  }

  if (readError) {
    throw new Error(
      `Message read states could not be loaded: ${readError.message}`,
    );
  }

  if (usersError) {
    throw new Error(
      `Messaging users could not be loaded: ${usersError.message}`,
    );
  }
  ...
  return buildConversations(
    ...
    (messages ?? []) as Message[],
    (reads ?? []) as MessageReadState[],
    (users ?? []) as MessagingUser[],
    userId,
  );
}
...
export async function getDirectConversationMessages(
  ...
  if (error) {
    throw new Error(`Messages could not be loaded: ${error.message}`);
  }
...
export async function startDirectConversation(
  ...
  if (error) {
    throw new Error(
      `Direct conversation could not be started: ${error.message}`,
    );
  }
...
function notifyDirectMessagePushBestEffort(
  conversationId: string,
  messageId: string,
): void {
  try {
...
export async function markConversationMessagesRead(
  ...
  if (error) {
    throw new Error(
      `Messages could not be marked read: ${error.message}`,
    );
  }
...
export async function getCompanyAnnouncements(
  userId: string,
): Promise<AnnouncementConversation[]> {
  const client = getSupabaseClient();
  ...
  if (conversationError) {
    throw new Error(
      `Announcements could not be loaded: ${conversationError.message}`,
    );
  }
  ...
  if (messageError) {
    throw new Error(
      `Announcement messages could not be loaded: ${messageError.message}`,
    );
  }
  ...
  if (ackError) {
    throw new Error(
      `Announcement acknowledgments could not be loaded: ${ackError.message}`,
    );
  }
  ...
  return buildAnnouncementConversations(
    rows,
    (messages ?? []) as Message[],
    (reads ?? []) as MessageReadState[],
    (acknowledgments ?? []) as AnnouncementAcknowledgment[],
    (users ?? []) as MessagingUser[],
    userId,
  );
}
...
export async function sendCompanyAnnouncement(
  ...
  if (!trimmedTitle) {
    throw new Error("Enter an announcement title before sending.");
  }

  if (!trimmedBody) {
    throw new Error("Enter an announcement message before sending.");
  }
  ...
  if (error) {
    throw new Error(`Announcement could not be sent: ${error.message}`);
  }
...
export async function acknowledgeRequiredAnnouncement(
  messageId: string,
): Promise<AnnouncementAcknowledgment> {
  const { data, error } = await getSupabaseClient().rpc(
    "acknowledge_required_announcement",
    {
  ...
  if (error) {
    throw new Error(
      `Announcement could not be acknowledged: ${error.message}`,
    );
  }
...
function buildAnnouncementConversations(
  conversations: Conversation[],
  messages: Message[],
  reads: MessageReadState[],
  ...
  return conversations.map((conversation) => {
    const conversationMessages: AnnouncementMessage[] = messages
      ...
      .map((message) => ({
        ...message,
        sender:
          usersById.get(message.sender_user_id) ?? null,
        acknowledgedAt:
          acknowledgedIds.get(message.id) ?? null,
      }));
...
function buildConversations(
  ...
  return conversations.map((conversation) => {
    ...
    const participantId = conversationMembers.find(
