...
export function isPastDueInvoice(x:InvoiceWithRelations){return Boolean(x.due_date&&x.due_date<localDate()&&x.balance_due>0&&!["Paid","Cancelled","Archived"].includes(x.status))}
export async function markPastDueInvoices(rows:InvoiceWithRelations[]){const due=rows.filter(x=>isPastDueInvoice(x)&&x.status!=="Past Due");if(!due.length)return false;const{error}=await getSupabaseClient().from("invoices").update({status:"Past Due"}).in("id",due.map(x=>x.id));if(error)throw error;return true}
