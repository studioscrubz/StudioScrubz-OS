...
const visitsPerMonth: Partial<Record<AgreementFrequency, number>> = { Daily: 30.42, Weekly: 4.33, Biweekly: 2.17, Monthly: 1 };
...
export function catalogAgreementPricing(input: { standardPrice:number; frequency:AgreementFrequency; serviceId:string; rules:RecurringPricingRule[] }): AgreementPricingSnapshot {
  const recurring = supported.has(input.frequency)
    ? applyRecurringRule(input.standardPrice, input.frequency as "One-Time"|"Weekly"|"Biweekly"|"Monthly", input.rules, input.serviceId)
    : { amount: input.standardPrice, discount: 0, percent: 0 };
  const final = round(recurring.amount);
  return make({ source:"Service Catalog", standard:input.standardPrice, frequency:input.frequency, frequencyDiscount:recurring.discount, frequencyPercent:recurring.percent, customDiscount:0, taxes:0, final, monthly:input.frequency === "One-Time" ? null : round(final * (visitsPerMonth[input.frequency] ?? 0)) });
}
