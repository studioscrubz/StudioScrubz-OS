...
function make(values:{source:AgreementPricingSnapshot["source"];serviceDescription?:string|null;standard:number;frequency:AgreementFrequency;recurringRuleId?:string|null;recurringRuleName?:string|null;frequencyDiscount:number;frequencyPercent:number;customDiscount:number;taxes:number;final:number;monthly:number|null;acceptedPricingAllocation?:AgreementPricingSnapshot["accepted_pricing_allocation"];upkeepPlan?:AgreementPricingSnapshot["upkeep_plan"];catalogAddons?:AgreementPricingSnapshot["catalog_addons"]}):AgreementPricingSnapshot {
