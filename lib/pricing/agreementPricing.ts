...
export function catalogAgreementPricing(input: { standardPrice:number; frequency:AgreementFrequency; serviceId:string; rules:RecurringPricingRule[]; customDiscount?:number; serviceDescription?:string|null }): AgreementPricingSnapshot {
...
function make(values:{source:AgreementPricingSnapshot["source"];serviceDescription?:string|null;standard:number;frequency:AgreementFrequency;frequencyDiscount:number;frequencyPercent:number;customDiscount:number;taxes:number;final:number;monthly:number|null;catalogAddons?:AgreementPricingSnapshot["catalog_addons"]}):AgreementPricingSnapshot {
  return { source:values.source, service_description:values.serviceDescription?.trim()||null, standard_service_price:round(values.standard), frequency:values.frequency, frequency_discount_label:`${values.frequency} Service Discount`, frequency_discount_percent:round(values.frequencyPercent), frequency_discount_amount:round(values.frequencyDiscount), price_after_frequency_discount:round(Math.max(0,values.standard-values.frequencyDiscount)), custom_discount_amount:round(values.customDiscount), taxes:round(values.taxes), final_per_visit_price:round(values.final), estimated_monthly_total:values.monthly===null?null:round(values.monthly), captured_at:new Date().toISOString(),catalog_addons:values.catalogAddons??[] };
}
