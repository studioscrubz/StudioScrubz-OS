...
export function calculateResidentialEstimate(input: ResidentialCalculatorInput, catalog: ServiceCatalogBundle, upkeepAdjustmentPercent = 30): EstimateResult {
  ...
  return result({ input, serviceName: `${input.serviceType} Cleaning`, catalogAddons:snapshots(input.addOns,availableAddons,input.addonSelections), basePrice, adjustments, oneTimePrice, recurringPricingRuleId:pricing.recurringPricingRuleId, recurringPricingRuleName:pricing.recurringPricingRuleName, recurringDiscount, recurringDiscountPercent, manualDiscount, totalDiscount, taxes, finalPrice, laborHours, crewSize, laborCost, supplyCost, scope: [] });
}
...
export function calculateCommercialEstimate(input: CommercialCalculatorInput, catalog: ServiceCatalogBundle, resolvedService?:ServiceCatalogBundle["services"][number], addonDivision:"Residential"|"Commercial"="Commercial"): EstimateResult {
  ...
  const taxes = pricing.taxes;
  const finalPrice = pricing.finalPrice;
  return result({ input, serviceName: `${input.commercialType} Cleaning`, catalogAddons:snapshots(input.additionalServices,availableAddons,input.addonSelections), basePrice, adjustments, oneTimePrice, recurringPricingRuleId:pricing.recurringPricingRuleId, recurringPricingRuleName:pricing.recurringPricingRuleName, recurringDiscount, recurringDiscountPercent, manualDiscount, totalDiscount, taxes, finalPrice, laborHours, crewSize, laborCost, supplyCost, scope: [] });
}
...
function snapshots(names:string[],addons:ServiceCatalogBundle["addons"],selected:EstimateResult["calculatorInput"]["addonSelections"]):EstimateResult["catalogAddons"]{return names.map(name=>addons.find(addon=>addon.addon_name===name)).filter((addon):addon is NonNullable<typeof addon>=>Boolean(addon)).map(addon=>selected?.find(item=>item.catalogAddonId===addon.id)??{id:addon.id,catalogAddonId:addon.id,name:addon.addon_name,description:addon.description,price:addon.price,pricingModel:addon.pricing_model,unitLabel:addon.unit_label})}
