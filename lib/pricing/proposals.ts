...
export function calculateProposal(input: { estimate: EstimateResult | null; catalogBasePrice?: number; recurringRules: RecurringPricingRule[]; recurringPricingRuleId?:string|null; serviceId?: string; serviceName: string; serviceDescription: string | null; frequency: Frequency; customIntervalDays?: number | null; estimatedCleaningDays?:number|null; estimatedHoursPerDay?:number|null; adjustments: ProposalAdjustment[]; additionalLabor: number; additionalMaterials: number; manualDiscountPercent: number; scope: ProposalScopeItem[]; terms: ProposalTerms }): ProposalResult {
  ...
  return { serviceName: input.serviceName, serviceDescription: input.serviceDescription, customIntervalDays, estimatedCleaningDays:input.estimatedCleaningDays??null, estimatedHoursPerDay:input.estimatedHoursPerDay??null, upkeepPlan: input.estimate?.upkeepPlan ?? null, baseEstimateAmount: money(input.estimate?.finalPrice ?? input.catalogBasePrice ?? beforeDiscount), adjustments: input.adjustments, additionalLabor: money(input.additionalLabor), additionalMaterials: money(input.additionalMaterials), recurringPricingRuleId,recurringPricingRuleName, frequencyDiscount: money(recurringDiscount), frequencyDiscountPercent: money(recurringDiscountPercent), inheritedManualDiscount: input.estimate?.manualDiscount ?? 0, manualDiscount: money(manualDiscount), taxRate: 0, taxes: money(taxes), taxFreePricing: true, perVisitTotal: money(perVisitTotal), monthlyTotal: input.estimate?.upkeepPlan ? input.estimate.upkeepPlan.monthlyPackage : estimatedMonthlyTotal(perVisitTotal, input.frequency, customIntervalDays), laborHours: Math.round(laborHours * 10) / 10, crewRecommendation: crew, estimatedDuration: duration, estimatedProfit: money(perVisitTotal - costs), scope: input.scope, terms: input.terms };
}
