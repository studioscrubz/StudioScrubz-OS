...
function ContractBilling({agreement,canManage,close}:{agreement:AgreementWithRelations;canManage:boolean;close:()=>void}) {
  const [summary,setSummary]=useState<AgreementFinancialSummary|null>(null),[invoices,setInvoices]=useState<InvoiceWithRelations[]>([]),[month,setMonth]=useState(dateOnly().slice(0,7)),[amount,setAmount]=useState(0),[busy,setBusy]=useState(false),[error,setError]=useState<string|null>(null);
  ...
  async function create(){setBusy(true);setError(null);try{if(agreement.billing_type==="Monthly")await createMonthlyContractInvoice({agreementId:agreement.id,billingMonth:month});else await createFlatContractInvoice({agreementId:agreement.id,amount});await load()}catch(cause){setError(message(cause))}finally{setBusy(false)}}
