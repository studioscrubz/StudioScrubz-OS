...
export function AgreementsPage() {
  ...
  return <>
    ...
    {send && <SendAgreementModal agreement={send} settings={settings} sender={profile?.display_name || profile?.email || "Master Admin"} close={() => setSend(null)} sent={async () => {
      setSend(null); await load(); setNotice("Resend accepted the Agreement email.");
    }}/>}
  </>;
}
...
function SendAgreementModal({ agreement, settings, sender, close, sent }: { agreement: AgreementWithRelations; settings: BusinessSettings | null; sender: string; close: () => void; sent: () => Promise<void> }) {
  ...
  async function submit() {
    ...
    try {
      ...
      await sendTransactionalCustomerEmail({ documentType: "Service Agreement", documentId: agreement.id, recipientEmail: email, subject, messageBody: body, requestId: crypto.randomUUID() });
      await sent();
    } catch (cause) { setError(message(cause)); } finally { setBusy(false); }
  }
  return <Modal close={close}><h2 className="text-2xl font-bold text-[#143d1a]">Send Service Agreement</h2><p className="mt-1 text-sm text-neutral-500">Send delivers this secure Agreement link through StudioScrubz transactional email.</p>
    ...
    <div className="mt-5 rounded-xl border bg-neutral-50 p-4"><AgreementDocument agreement={agreement} settings={settings}/></div>{error && <Alert text={error}/>}<button className={`${primary} mt-5`} disabled={busy} onClick={() => void submit()}>{busy ? "Sending…" : "Send"}</button>
  </Modal>;
}
...
function AgreementForm({ value, proposal, settings, close, saved }: { value: AgreementWithRelations | null; proposal: ProposalWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  ...
  const pricing=useMemo(()=>{if(proposal)return{...proposalAgreementPricing(proposal),service_description:agreementServiceDescriptionFromProposal(proposal,services)};if(value?.proposal_id||(value&&!pricingDirty))return value.pricing_snapshot;const service=services.find((item)=>item.id===selectedServiceId);return service?catalogAgreementPricing({standardPrice:basePrice,frequency:draft.frequency,serviceId:service.id,rules,customDiscount:manualDiscount,serviceDescription:service.description}):value?.pricing_snapshot??null},[basePrice,draft.frequency,manualDiscount,pricingDirty,proposal,rules,selectedServiceId,services,value]);
  ...
  function selectService(serviceId: string) { const service = services.find((item) => item.id === serviceId); if (!service) return; setSelectedServiceId(service.id); set("service_name", service.service_name); if (!operationalEdit) setPricingDirty(true); }
