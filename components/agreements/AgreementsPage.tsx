...
function AgreementDocument({ agreement, settings }: { agreement: AgreementWithRelations; settings: BusinessSettings | null }) {
  ...
  const schedule = ["Weekly", "Biweekly", "Every 4 Weeks", "Multiple Days Per Week"].includes(agreement.frequency) && agreement.days_of_week.length ? agreement.days_of_week.map((day) => ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][day]).join(", ") : agreement.frequency === "Twice Monthly" ? `Days ${agreement.day_of_month} and ${agreement.second_day_of_month} of each month` : agreement.frequency === "Monthly" ? `Day ${agreement.day_of_month} of each month` : agreement.frequency === "Custom" ? `Every ${agreement.custom_interval_days} days` : agreement.frequency;
...
function AgreementForm({ value, proposal, settings, close, saved }: { value: AgreementWithRelations | null; proposal: ProposalWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  ...
  const clientRows = clients.filter((client) => client.client_type === draft.division || client.client_type === "Contractor" || client.id === draft.client_id);
...
function RecurrenceControls({ draft, set }: { draft: AgreementInput; set: <K extends keyof AgreementInput>(key: K, value: AgreementInput[K]) => void }) {
  ...
  return <div className="mt-4 rounded-xl border border-green-100 bg-green-50/50 p-4"><h3 className="font-bold text-[#143d1a]">Recurrence Schedule</h3>
    ...
    {draft.frequency === "Twice Monthly" && <div className="grid max-w-xl gap-3 sm:grid-cols-2"><Field label="First Day of Month (1–28)" type="number" value={draft.day_of_month === null ? "" : String(draft.day_of_month)} set={(next) => set("day_of_month", next ? Number(next) : null)}/><Field label="Second Day of Month (1–28)" type="number" value={draft.second_day_of_month === null ? "" : String(draft.second_day_of_month)} set={(next) => set("second_day_of_month", next ? Number(next) : null)}/><p className="sm:col-span-2 text-xs text-neutral-600">Two distinct calendar dates are required. Limiting dates to 1–28 guarantees exactly two visits in every month, including February.</p></div>}
    ...
    {!usesWeekdays && !["Monthly", "Twice Monthly", "Custom"].includes(draft.frequency) && <p className="mt-2 text-sm text-neutral-600">{draft.frequency === "One-Time" ? "One service date will be scheduled from the agreement start date." : "Service repeats daily from the agreement start date."}</p>}
  </div>;
}
