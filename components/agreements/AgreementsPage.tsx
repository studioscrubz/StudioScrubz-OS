...
import {
  activateAgreement, archiveAgreement, cancelAgreement, completeAgreement, createAgreement, createAgreementFromProposal,
...
export function AgreementsPage() {
  ...
  return <>
    ...
    <Occurrences rows={visibleOccurrences.filter((row) => row.scheduled_date >= today)} act={act} canManage={canManage}/>
...
function Occurrences({ rows, act, canManage }: { rows: ServiceOccurrenceWithRelations[]; act: (operation: () => Promise<unknown>, success: string) => Promise<void>; canManage: boolean }) { function remove(row:ServiceOccurrenceWithRelations){const date=new Date(`${row.scheduled_date}T12:00:00`).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"});if(!window.confirm(`Permanently delete this upcoming service on ${date}?\n\nThis removes the scheduled occurrence and cannot be undone.`))return;void act(()=>deleteOccurrence(row.id),"Upcoming service deleted.")}return <section className="mt-10"><h2 className="text-xl font-bold">Upcoming Services</h2><div className="mt-3 overflow-x-auto rounded-xl border bg-white"><table className="w-full min-w-[900px] text-sm"><thead><tr>{["Date", "Time", "Agreement", "Service", "Status", "Job", "Actions"].map((label) => <th className="p-3 text-left" key={label}>{label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr className="border-t" key={row.id}><td className="p-3">{row.scheduled_date}</td><td className="p-3">{formatTime12Hour(row.scheduled_start_time)}</td><td className="p-3">{row.agreement.agreement_number}</td><td className="p-3">{row.agreement.service_name}</td><td className="p-3">{row.job?.status === "Completed" ? "Completed" : row.status}</td><td className="p-3">{row.job?.job_number || "—"}</td><td className="p-3"><div className="flex flex-wrap gap-1">{canManage && !row.job_id && row.status === "Scheduled" && <button className={secondary} onClick={() => void act(() => createJobFromOccurrence(row.id), "Job created.")}>Create Job</button>}{row.job_id && <a className={secondary} href={`/jobs?jobId=${row.job_id}`}>View Job</a>}{canManage && <><button className={secondary} onClick={() => { const next = prompt("New date", row.scheduled_date); if (next) void act(() => rescheduleOccurrence(row.id, next, row.scheduled_start_time), "Rescheduled."); }}>Reschedule</button><button className={secondary} onClick={() => void act(() => skipOccurrence(row.id), "Skipped.")}>Skip</button><button className={secondary} onClick={() => void act(() => cancelOccurrence(row.id), "Cancelled.")}>Cancel</button></>}{canManage&&!row.job_id&&row.status==="Scheduled"&&<button className="rounded-lg border border-red-300 px-3 py-2 text-xs font-bold text-red-700" onClick={()=>remove(row)}>Delete Upcoming Service</button>}</div></td></tr>)}</tbody></table>{!rows.length && <p className="p-8 text-center">No upcoming services.</p>}</div></section>; }
...
function AgreementForm({ value, proposal, settings, close, saved }: { value: AgreementWithRelations | null; proposal: ProposalWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  ...
  function selectService(serviceId: string) { const service = services.find((item) => item.id === serviceId); if (!service) return; const replacement = service.description ?? ""; const replaceScope = service.service_name !== draft.service_name || !scopeText.trim(); setSelectedServiceId(service.id); set("service_name", service.service_name); if (!operationalEdit) setPricingDirty(true); if (replaceScope) setScopeText(replacement); }
  ...
  async function save() { try { setError(null); setSaving(true); if(proposal&&!billingConfirmed)throw new Error("Select and confirm a Billing Type."); const contract=["Monthly","Flat Contract"].includes(draft.billing_type); const nextDraft = { ...draft, billing_amount:operationalEdit&&value?value.billing_amount:contract?basePrice:pricing?.final_per_visit_price??draft.billing_amount, pricing_snapshot:operationalEdit&&value?value.pricing_snapshot:pricing, scope: scopeFromText(scopeText, value?.scope[0]?.id) }; if(proposal){await createAgreementFromProposal(proposal.id,{startDate:nextDraft.start_date,endDate:nextDraft.end_date,daysOfWeek:nextDraft.days_of_week,intervalWeeks:nextDraft.interval_weeks,dayOfMonth:nextDraft.day_of_month,customIntervalDays:nextDraft.custom_interval_days,billingType:nextDraft.billing_type,billingAmount:nextDraft.billing_type==="Per Visit"?null:basePrice,assignedCrewId:nextDraft.assigned_crew_id,defaultStartTime:nextDraft.default_start_time});await saved();return} const selectedClient=clients.find((row)=>row.id===nextDraft.client_id); const selectedProperty=properties.find((row)=>row.id===nextDraft.property_id); const selectedService=services.find((row)=>row.id===selectedServiceId); const validation=validateAgreementConfiguration(nextDraft,selectedClient,selectedProperty,selectedService,Boolean(value&&["Active","Paused"].includes(value.status))); if(validation)throw new Error(validation); if (value) await saveAgreementEdits(value.id, nextDraft); else await createAgreement(nextDraft); await saved(); } catch (cause) { setError(message(cause)); } finally { setSaving(false); } }
  return <Modal close={close}><h2 className="mb-4 text-2xl font-bold text-[#143d1a]">{proposal ? "Review Accepted Proposal Agreement" : value ? "Edit Agreement" : "New Agreement"}</h2>{proposal&&<div className="mb-5 rounded-xl border border-[#d4af37]/50 bg-[#fffdf4] p-4"><p className="font-bold text-[#143d1a]">Accepted Proposal Authority</p><div className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><p><b>Customer Requested Service Date:</b> {proposal.requested_date||"—"}</p><p><b>Accepted Per-Visit Price:</b> {money(proposal.result.perVisitTotal)}</p><p><b>Service Description:</b> {proposal.result.serviceDescription||"—"}</p><p><b>Frequency:</b> {proposal.frequency}</p></div><p className="mt-3 text-xs text-neutral-600">The requested date is customer context, not a confirmed schedule. Confirm the Agreement Start Date and recurrence below.</p>{proposal.result.adjustments.filter(item=>item.catalogAddonId).length>0&&<div className="mt-3 flex flex-wrap gap-2">{proposal.result.adjustments.filter(item=>item.catalogAddonId).map(item=><span key={item.id} className="rounded-full bg-white px-3 py-1 text-xs font-semibold">{item.label}</span>)}</div>}</div>}<div className="grid gap-3 sm:grid-cols-2">
    ...
    <label className="sm:col-span-2">Scope / Service Details<textarea className="mt-1 min-h-36 w-full rounded-lg border px-3 py-3" value={scopeText} disabled={signedEdit||Boolean(proposal)} onChange={(event) => setScopeText(event.target.value)}/></label>
    <Select disabled={Boolean(proposal)} label="Frequency" value={draft.frequency} set={(next) => selectFrequency(next as AgreementInput["frequency"])} rows={AGREEMENT_FREQUENCIES.map((row) => [row, row])}/><div/>
    <Field label="Agreement Start Date" type="date" value={draft.start_date} set={(next) => set("start_date", next)}/><Field label="End Date" type="date" value={draft.end_date || ""} set={(next) => set("end_date", next || null)}/><Select disabled={operationalEdit} label="Billing Type (Required)" value={proposal&&!billingConfirmed?"":draft.billing_type} set={(next) => {set("billing_type",next as AgreementInput["billing_type"]);setBillingConfirmed(Boolean(next));if(proposal)setBasePrice(next==="Per Visit"?proposal.result.perVisitTotal:0)}} rows={AGREEMENT_BILLING_TYPES.map((row) => [row, row])}/><Field label={billingLabel(draft.billing_type)} type="number" value={String(basePrice)} set={(next)=>{setBasePrice(Number(next));setPricingDirty(true)}} disabled={operationalEdit||(Boolean(value?.proposal_id)&&!["Monthly","Flat Contract"].includes(draft.billing_type))||(Boolean(proposal)&&draft.billing_type==="Per Visit")}/><Field label="Manual / Custom Discount" type="number" value={String(manualDiscount)} set={(next)=>{setManualDiscount(Number(next));setPricingDirty(true)}} disabled={operationalEdit||Boolean(value?.proposal_id)||Boolean(proposal)||["Monthly","Flat Contract"].includes(draft.billing_type)}/>
  ...
  </div>{value&&["Active","Paused"].includes(value.status)&&<p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Changes affect future operations only. Existing jobs, invoices, payments, signed snapshots, and historical occurrences remain unchanged.</p>}{!["Monthly","Flat Contract"].includes(draft.billing_type)&&<AgreementPricingBreakdown pricing={pricing}/>}<RecurrenceControls draft={draft} set={set}/><BillingGuidance billingType={draft.billing_type}/><label className="mt-3 block"><input type="checkbox" checked={draft.auto_renew} onChange={(event) => set("auto_renew", event.target.checked)}/> Auto Renew</label>{error && <Alert text={error}/>}<button disabled={saving} className={`${primary} mt-4`} onClick={() => void save()}>{saving?"Saving...":"Save Agreement"}</button></Modal>;
}
