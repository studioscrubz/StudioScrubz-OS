...
function AgreementForm({ value, settings, close, saved }: { value: AgreementWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  ...
  async function save() { try { const nextDraft = { ...draft, billing_amount:pricing?.final_per_visit_price??draft.billing_amount, pricing_snapshot:pricing, scope: scopeFromText(scopeText, value?.scope[0]?.id) }; if (value) await updateAgreement(value.id, nextDraft); else await createAgreement(nextDraft); await saved(); } catch (cause) { setError(message(cause)); } }
  return <Modal close={close}><h2 className="mb-4 text-2xl font-bold text-[#143d1a]">{value ? "Edit Agreement" : "New Agreement"}</h2><div className="grid gap-3 sm:grid-cols-2">
    ...
    <Field label="Agreement Name" value={draft.agreement_name} set={(next) => set("agreement_name", next)}/><Select label="Division" value={draft.division} set={(next) => {set("division", next as AgreementInput["division"]);setSelectedServiceId("");set("service_name","");setPricingDirty(true);}} rows={[["Residential", "Residential"], ["Commercial", "Commercial"]]}/><Select label="Service" value={selectedServiceId} set={selectService} rows={[...(value && !services.some((service) => service.service_name === value.service_name) ? [["historical", `${value.service_name} (historical)`]] : []), ...services.filter((service)=>service.division===draft.division||service.division==="Both").map((service) => [service.id, `${service.service_name} · ${service.category} · ${service.division}`])]}/><div/>
    ...
    <Select label="Frequency" value={draft.frequency} set={(next) => {if(!value?.proposal_id){set("frequency", next as AgreementInput["frequency"]);setPricingDirty(true);}}} rows={AGREEMENT_FREQUENCIES.map((row) => [row, row])}/><div/>
