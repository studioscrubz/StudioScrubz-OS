...
function AgreementForm({ value, close, saved }: { value: AgreementWithRelations | null; close: () => void; saved: () => Promise<void> }) {
  ...
  function selectService(serviceId: string) { const service = services.find((item) => item.id === serviceId); if (!service) return; const replacement = service.description ?? ""; const replaceScope = service.service_name !== draft.service_name || !scopeText.trim(); setSelectedServiceId(service.id); set("service_name", service.service_name); if (replaceScope) setScopeText(replacement); }
