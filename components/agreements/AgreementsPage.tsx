...
function AgreementForm({ value, settings, close, saved }: { value: AgreementWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  ...
  return <Modal close={close}><h2 className="mb-4 text-2xl font-bold text-[#143d1a]">{value ? "Edit Agreement" : "New Agreement"}</h2><div className="grid gap-3 sm:grid-cols-2">
    <Select disabled={operationalEdit} label={draft.division === "Commercial" ? "Company / Client" : "Client"} value={draft.client_id || ""} set={(next) => { set("client_id", next); set("property_id", ""); }} rows={clientRows.map((row) => [row.id, clientOption(row)])}/><Select disabled={operationalEdit} label={draft.division === "Commercial" ? "Commercial Property / Site" : "Property"} value={draft.property_id || ""} set={(next) => set("property_id", next)} rows={propertyRows.map((row) => [row.id, row.property_name || row.address])}/>
    <Field label="Agreement Name" value={draft.agreement_name} set={(next) => set("agreement_name", next)}/><Select disabled={operationalEdit} label="Division" value={draft.division} set={(next) => {setDraft((current)=>({...current,division:next as AgreementInput["division"],client_id:"",property_id:"",service_name:""}));setSelectedServiceId("");setPricingDirty(true);}} rows={[["Residential", "Residential"], ["Commercial", "Commercial"]]}/><Select disabled={signedEdit} label="Service" value={selectedServiceId} set={selectService} rows={[...(value && !services.some((service) => service.service_name === value.service_name) ? [["historical", `${value.service_name} (historical)`]] : []), ...services.filter((service)=>service.division===draft.division||service.division==="Both").map((service) => [service.id, `${service.service_name} · ${service.category} · ${service.division}`])]}/><div/>
    ...
    <label className="sm:col-span-2">Scope / Service Details<textarea className="mt-1 min-h-36 w-full rounded-lg border px-3 py-3" value={scopeText} disabled={signedEdit} onChange={(event) => setScopeText(event.target.value)}/></label>
