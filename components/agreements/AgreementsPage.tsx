...
import { AGREEMENT_BILLING_TYPES, AGREEMENT_FREQUENCIES, AGREEMENT_STATUSES, type AgreementInput, type AgreementWithRelations, type Weekday } from "@/types/agreement";
...
export function AgreementsPage() {
  ...
  useOperationalRealtime(["service_agreements", "service_occurrences", "jobs"], load);
  useEffect(() => {
...
function AgreementForm({ value, settings, close, saved }: { value: AgreementWithRelations | null; settings: BusinessSettings | null; close: () => void; saved: () => Promise<void> }) {
  const [draft, setDraft] = useState<AgreementInput>(value ? pick(value) : blank(settings?.default_service_agreement_terms)); const [clients, setClients] = useState<Client[]>([]); const [properties, setProperties] = useState<PropertyWithClient[]>([]); const [crews, setCrews] = useState<CrewWithRelations[]>([]); const [services, setServices] = useState<CatalogService[]>([]); const [rules, setRules] = useState<RecurringPricingRule[]>([]); const [selectedServiceId, setSelectedServiceId] = useState(""); const [basePrice,setBasePrice]=useState(value?.pricing_snapshot?.standard_service_price??value?.billing_amount??0); const [manualDiscount,setManualDiscount]=useState(value?.pricing_snapshot?.custom_discount_amount??0); const [taxes,setTaxes]=useState(value?.pricing_snapshot?.taxes??0); const [pricingDirty,setPricingDirty]=useState(false); const [scopeText, setScopeText] = useState(scopeToText(value?.scope ?? [])); const [error, setError] = useState<string | null>(null);
  ...
  const pricing=useMemo(()=>{if(value?.proposal_id||(value&&!pricingDirty))return value.pricing_snapshot;const service=services.find((item)=>item.id===selectedServiceId);return service?catalogAgreementPricing({standardPrice:basePrice,frequency:draft.frequency,serviceId:service.id,rules,customDiscount:manualDiscount,taxes}):value?.pricing_snapshot??null},[basePrice,draft.frequency,manualDiscount,pricingDirty,rules,selectedServiceId,services,taxes,value]);
  ...
  return <Modal close={close}><h2 className="mb-4 text-2xl font-bold text-[#143d1a]">{value ? "Edit Agreement" : "New Agreement"}</h2><div className="grid gap-3 sm:grid-cols-2">
    ...
    <Field label="Start Date" type="date" value={draft.start_date} set={(next) => set("start_date", next)}/><Field label="End Date" type="date" value={draft.end_date || ""} set={(next) => set("end_date", next || null)}/><Select label="Billing Type" value={draft.billing_type} set={(next) => set("billing_type", next as AgreementInput["billing_type"])} rows={AGREEMENT_BILLING_TYPES.map((row) => [row, row])}/><Field label="Base Service Price" type="number" value={String(basePrice)} set={(next)=>{setBasePrice(Number(next));setPricingDirty(true)}} disabled={Boolean(value?.proposal_id)}/><Field label="Manual / Custom Discount" type="number" value={String(manualDiscount)} set={(next)=>{setManualDiscount(Number(next));setPricingDirty(true)}} disabled={Boolean(value?.proposal_id)}/><Field label="Taxes" type="number" value={String(taxes)} set={(next)=>{setTaxes(Number(next));setPricingDirty(true)}} disabled={Boolean(value?.proposal_id)}/>
  ...
  </div><AgreementPricingBreakdown pricing={pricing}/><RecurrenceControls draft={draft} set={set}/><BillingGuidance billingType={draft.billing_type}/><label className="mt-3 block"><input type="checkbox" checked={draft.auto_renew} onChange={(event) => set("auto_renew", event.target.checked)}/> Auto Renew</label>{error && <Alert text={error}/>}<button className={`${primary} mt-4`} onClick={() => void save()}>Save Agreement</button></Modal>;
}
...
function BillingGuidance({ billingType }: { billingType: AgreementInput["billing_type"] }) {
  ...
  return <p className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900"><b>{billingType}:</b> billing amount represents the {contract}. Automated contract invoicing is not implemented; generated occurrence jobs currently use agreement billing amounts, so review billing manually. Commercial Monthly and Flat Contract agreements cannot be activated yet.</p>;
}

function blank(defaultAgreementTerms?: string | null): AgreementInput { return { client_id: "", property_id: "", proposal_id: null, division: "Residential", agreement_name: "", service_name: "", frequency: "Weekly", days_of_week: [], interval_weeks: 1, day_of_month: null, custom_interval_days: null, start_date: dateOnly(), end_date: null, auto_renew: false, billing_type: "Per Visit", billing_amount: 0, payment_terms: null, agreement_terms: defaultAgreementTerms || null, cancellation_terms: null, scope: [], special_instructions: null, assigned_crew_id: null, default_start_time: null, estimated_duration: null, status: "Draft", notes: null }; }
