...
export function PublicAgreementPage({ token }: { token:string }) {
  const [agreement,setAgreement]=useState<PublicAgreement|null>(null),[name,setName]=useState(""),[consent,setConsent]=useState(false),[loading,setLoading]=useState(true),[signing,setSigning]=useState(false),[error,setError]=useState<string|null>(null);
  ...
  const signed=Boolean(agreement.client_signed_at),canSign=agreement.status==="Sent"&&!signed;
  return <PublicShell><article id="public-agreement" className="rounded-2xl border bg-white p-6 shadow-sm print:border-0 print:p-0 print:shadow-none">
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2"><Detail label="Client" value={agreement.client_name}/><Detail label="Property / Service Location" value={agreement.property_location}/><Detail label="Service" value={agreement.service_name}/><Detail label="Frequency" value={agreement.frequency}/><Detail label="Service Days" value={schedule}/><Detail label="Service Time" value={formatTime12Hour(agreement.default_start_time)}/><Detail label="Start Date" value={agreement.start_date}/>{agreement.end_date&&<Detail label="End Date" value={agreement.end_date}/>}<Detail label="Billing" value={`${money(agreement.billing_amount)} · ${agreement.billing_type}`}/></div>
