...
export function ScheduleOccurrences() {
  ...
  async function load() {
    ...
    setRows(records.filter((row) => row.agreement.status === "Active" && !row.job_id && !["Skipped", "Cancelled"].includes(row.status)));
    setError(false);
  }
  ...
  useEffect(() => {
    ...
    void getUpcomingOccurrences(day(), add(day(), 60))
      .then((records) => { if (active) setRows(records.filter((row) => row.agreement.status === "Active" && !row.job_id && !["Skipped", "Cancelled"].includes(row.status))); })
