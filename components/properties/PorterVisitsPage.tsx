...
import { getPorterAssignmentOptions } from "@/lib/services/employees";
...
import type { PorterAssignmentOption } from "@/types/employee";
...
export function PorterVisitsPage() {
  ...
  const [assignmentOptions, setAssignmentOptions] = useState<PorterAssignmentOption[]>([]);
  ...
  useEffect(() => {
    if (!allowed) return;
    let active = true;
    Promise.all([listPorterVisits(), management ? listPropertyServicePlans() : Promise.resolve([]), management ? getActiveCrews() : Promise.resolve([]), management ? getPorterAssignmentOptions() : Promise.resolve([])])
      .then(([rows, planRows, crewRows, options]) => { if (active) { setVisits(rows); setPlans(planRows.filter(plan => plan.status === "Active" && !plan.archived_at)); setCrews(crewRows); setAssignmentOptions(options); } })
  ...
  async function refresh() {
    ...
    try {
      ...
      if (management) { const [planRows, crewRows, options] = await Promise.all([listPropertyServicePlans(), getActiveCrews(), getPorterAssignmentOptions()]); setPlans(planRows.filter(plan => plan.status === "Active" && !plan.archived_at)); setCrews(crewRows); setAssignmentOptions(options); }
  ...
  const filtered = visits.filter(visit => (status === "All" || visit.status === status) && (!date || visit.scheduled_date === date)
    && [visit.plan_name, visit.property_label, visit.crew_name, visit.worker_name, visit.manager_name, visit.scheduled_date].join(" ").toLowerCase().includes(search.toLowerCase()));
  return <>
    ...
    {creating && management && <CreateVisit plans={plans} crews={crews} options={assignmentOptions} busy={busy} submit={create} close={() => setCreating(false)}/>}
    {selected && <VisitDetail key={`${selected.id}:${selected.updated_at}`} visit={selected} crews={crews} options={assignmentOptions} management={management} busy={busy} mutate={mutate} remove={() => removeVisit(selected)} reload={() => open(selected.id)} close={() => setSelected(null)}/>}
...
function AssignmentFields({ crews, options, workerEmployeeId, workerCrewId, managerId, change }: { crews: CrewWithRelations[]; options: PorterAssignmentOption[]; workerEmployeeId: string | null; workerCrewId: string | null; managerId: string | null; change: (value: Partial<Pick<CreatePorterVisitInput, "assigned_worker_employee_id" | "assigned_worker_crew_id" | "assigned_manager_employee_id">>) => void }) {
  const techs = options.filter(option => option.service_role === "Porter Tech");
  const managers = options.filter(option => option.service_role === "Porter Manager");
  return <div className="rounded-xl border border-neutral-200 p-4"><p className="text-sm font-extrabold text-[#143d1a]">Worker assignment</p><p className="mt-1 text-xs text-neutral-600">Choose an individual Porter Tech or a Porter Crew. Selecting one clears the other.</p><div className="mt-3 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold">Individual Porter Tech<select className={field} value={workerEmployeeId ?? ""} onChange={e => change({ assigned_worker_employee_id: e.target.value || null, assigned_worker_crew_id: null })}><option value="">No individual tech</option>{workerEmployeeId && !techs.some(option => option.employee_id === workerEmployeeId) && <option value={workerEmployeeId}>Existing assigned tech</option>}{techs.map(option => <option key={option.employee_id} value={option.employee_id}>{option.display_name}</option>)}</select></label><label className="text-sm font-bold">Porter Crew<select className={field} value={workerCrewId ?? ""} onChange={e => change({ assigned_worker_crew_id: e.target.value || null, assigned_worker_employee_id: null })}><option value="">No crew</option><CrewOptions crews={crews} value={workerCrewId}/></select></label></div><label className="mt-4 block text-sm font-bold">Porter Manager (optional)<select className={field} value={managerId ?? ""} onChange={e => change({ assigned_manager_employee_id: e.target.value || null })}><option value="">No Porter Manager</option>{managerId && !managers.some(option => option.employee_id === managerId) && <option value={managerId}>Existing assigned manager</option>}{managers.map(option => <option key={option.employee_id} value={option.employee_id}>{option.display_name}</option>)}</select></label></div>;
}
function CreateVisit({ plans, crews, options, busy, submit, close }: { plans: PropertyServicePlanWithAreas[]; crews: CrewWithRelations[]; options: PorterAssignmentOption[]; busy: boolean; submit: (input: CreatePorterVisitInput) => Promise<void>; close: () => void }) {
  const [input, setInput] = useState<CreatePorterVisitInput>({ plan_id: "", scheduled_date: "", scheduled_start_time: "", assigned_worker_employee_id: null, assigned_worker_crew_id: null, assigned_manager_employee_id: null, visit_notes: null });
  ...
  return <form onSubmit={save} className="mt-6 rounded-2xl border bg-white p-6"><h2 className="text-xl font-extrabold text-[#143d1a]">New Porter Visit</h2><fieldset disabled={busy} className="mt-4 space-y-4">
    ...
    <AssignmentFields crews={crews} options={options} workerEmployeeId={input.assigned_worker_employee_id} workerCrewId={input.assigned_worker_crew_id} managerId={input.assigned_manager_employee_id} change={values => setInput(current => ({ ...current, ...values }))}/>
...
function VisitDetail({ visit, crews, options, management, busy: parentBusy, mutate, remove, reload, close }: { visit: PorterVisitWithAreas; crews: CrewWithRelations[]; options: PorterAssignmentOption[]; management: boolean; busy: boolean; mutate: (mutation: PorterVisitMutation) => Promise<void>; remove?: () => Promise<void>; reload: () => Promise<void>; close: () => void }) {
  ...
  const [workerEmployeeId, setWorkerEmployeeId] = useState(visit.assigned_worker_employee_id);
  const [workerCrewId, setWorkerCrewId] = useState(visit.assigned_worker_crew_id ?? visit.assigned_crew_id);
  const [managerId, setManagerId] = useState(visit.assigned_manager_employee_id);
  ...
  return <section className="mt-6 rounded-2xl border border-[#143d1a]/20 bg-white p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold text-[#9a7a17]">{visit.status} · {visitSchedule(visit)}</p><h2 className="mt-2 text-2xl font-extrabold text-[#143d1a]">{visit.property_label}</h2><p className="mt-2">{visit.plan_name} · Worker: {visit.worker_name ?? visit.crew_name ?? "Unassigned"}{visit.manager_name ? ` · Manager: ${visit.manager_name}` : ""}</p></div><button className={button} disabled={busy} onClick={close}>Close</button></div>
    <fieldset disabled={busy || terminal} className="mt-5 space-y-4">
      {canEditSchedule && <><div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-bold">Scheduled date<input required type="date" className={field} value={date} onChange={e => setDate(e.target.value)}/></label><label className="text-sm font-bold">Start time<input required type="time" className={field} value={startTime} onChange={e => setStartTime(e.target.value)}/></label></div><AssignmentFields crews={crews} options={options} workerEmployeeId={workerEmployeeId} workerCrewId={workerCrewId} managerId={managerId} change={values => { if ("assigned_worker_employee_id" in values) setWorkerEmployeeId(values.assigned_worker_employee_id ?? null); if ("assigned_worker_crew_id" in values) setWorkerCrewId(values.assigned_worker_crew_id ?? null); if ("assigned_manager_employee_id" in values) setManagerId(values.assigned_manager_employee_id ?? null); }}/></>}
      ...
      {!terminal && <button className={button} onClick={() => void mutate(canEditSchedule ? { action: "edit", data: { scheduled_date: date, scheduled_start_time: startTime, assigned_worker_employee_id: workerEmployeeId, assigned_worker_crew_id: workerCrewId, assigned_manager_employee_id: managerId, visit_notes: notes || null } } : { action: "notes", data: { visit_notes: notes || null } })}>Save {canEditSchedule ? "Visit Details" : "Notes"}</button>}
    </fieldset>
