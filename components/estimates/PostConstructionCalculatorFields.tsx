...
export function PostConstructionCalculatorFields({value,onChange,service,catalog}:{value:PostConstructionEstimateInput;onChange:(value:PostConstructionEstimateInput)=>void;service:CatalogService|undefined;catalog:ServiceCatalogBundle}){
  if (isPostConstructionV2Estimate(value)) return <div className="space-y-4">{value.division === "Residential" && <Group title="Residential Property Scope"><NumberField label="Bedrooms" value={value.rooms} set={rooms => onChange({ ...value, rooms })}/><NumberField label="Bathrooms" value={value.bathrooms} step="0.5" set={bathrooms => onChange({ ...value, bathrooms })}/><NumberField label="Floors" value={value.floors} set={floors => onChange({ ...value, floors })}/><NumberField label="Kitchens" value={value.kitchens} set={kitchens => onChange({ ...value, kitchens })}/><p className="text-xs text-neutral-600 sm:col-span-2 xl:col-span-4">Scope for walkthrough preparation and labor planning. Total square feet is entered below; these counts do not add pricing charges.</p></Group>}<ProjectCostFields value={value.projectCosting} onChange={projectCosting => onChange({ ...value, squareFeet: projectCosting.totalSquareFeet, projectCosting })}/></div>;
