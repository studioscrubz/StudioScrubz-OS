...
import { calculateCommercialEstimate, calculateResidentialEstimate, calculatePostConstructionCatalogEstimate, isPostConstructionV2Estimate } from "@/lib/pricing/estimates";
...
export function WalkthroughPricingReviewModal({ walkthrough, catalog, close, approved }: { walkthrough: WalkthroughWithRelations; catalog: ServiceCatalogBundle; close: () => void; approved: () => void }) {
  ...
  const v2 = isPostConstructionInput(input) && isPostConstructionV2Estimate(input);
  const result = useMemo(() => { try { return isPostConstructionInput(input) ? isPostConstructionV2Estimate(input) ? calculatePostConstructionCatalogEstimate(input, catalog) : null : input.division === "Residential" ? calculateResidentialEstimate(input, catalog) : calculateCommercialEstimate(input, catalog); } catch { return null; } }, [catalog, input]);
  async function approve() { setBusy(true); setError(null); try { const response = await fetch("/api/walkthroughs/pricing-review", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ walkthroughId: walkthrough.id, calculatorInput: input, ...(v2 ? {} : { manualPrice }) }) }); const body = await response.json() as { error?: string }; if (!response.ok) throw new Error(body.error || "Pricing could not be approved."); approved(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Pricing could not be approved."); } finally { setBusy(false); } }
  ...
  return <div className="fixed inset-0 z-[90] overflow-y-auto bg-[#07190a]/70 p-3 sm:p-6"><section role="dialog" aria-modal="true" aria-labelledby="pricing-review-title" className="mx-auto max-w-5xl rounded-2xl bg-[#f5f6f4] shadow-2xl"><header className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b bg-white px-5 py-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#9a7a17]">Internal financial review</p><h2 id="pricing-review-title" className="mt-1 text-xl font-extrabold text-[#143d1a]">Review Walkthrough Pricing</h2></div><button type="button" onClick={close} aria-label="Close pricing review" className="grid size-9 place-items-center rounded-lg border text-xl">×</button></header><div className="space-y-5 p-4 sm:p-7">
    ...
    <section className="rounded-2xl border bg-white p-5"><h3 className="font-extrabold text-[#143d1a]">Confirmed Calculator Inputs</h3><p className="mt-1 text-xs text-neutral-500">Walkthrough values take precedence; missing values fall back to the preliminary Estimate or active catalog defaults.</p><div className="mt-4">{isPostConstructionInput(input) ? isPostConstructionV2Estimate(input) ? <PostConstructionCalculatorFields value={input} onChange={setInput} catalog={catalog} service={findCatalogService(catalog.services, input.division, input.serviceType)} /> : <p role="alert" className="rounded-lg bg-neutral-50 p-3 text-sm text-neutral-600">Legacy Post-Construction snapshots cannot be approved through V2. Create an explicit V2 estimate before reviewing this assessment.</p> : input.division === "Residential" ? <ResidentialFields value={input} set={setInput} catalog={catalog} /> : <CommercialFields value={input} set={setInput} catalog={catalog} />}</div></section>
    <section className="rounded-2xl bg-[#143d1a] p-5 text-white"><p className="text-xs font-bold uppercase tracking-wider text-[#d4af37]">Reviewed Price</p>{result&&!v2&&<><p className="mt-2 text-sm text-white/65">Calculated reference: {money(result.finalPrice)}</p><label className="mt-4 block text-xs font-bold text-white/75">Approved Walkthrough Price<input aria-label="Approved walkthrough price" type="number" min="0" step="0.01" value={manualPrice ?? ""} placeholder={String(result.finalPrice)} onChange={event=>setManualPrice(event.target.value===""?null:Number(event.target.value))} className="mt-2 h-11 w-full rounded-lg border border-white/20 bg-white px-3 text-sm font-bold text-[#143d1a]" /></label><button type="button" disabled={manualPrice===null} onClick={()=>setManualPrice(null)} className="mt-2 text-xs font-bold text-[#d4af37] disabled:opacity-40">Use Calculated Price</button></>}<p className="mt-3 text-3xl font-extrabold">{result ? money(v2 ? result.finalPrice : manualPrice ?? result.finalPrice) : "Complete required pricing inputs"}</p><p className="mt-2 text-xs text-white/70">The server reloads the active catalog, recalculates the reference, and preserves this explicit approved amount. The original Estimate is not changed.</p>{error && <p role="alert" className="mt-4 rounded-lg bg-red-50 p-3 text-sm font-bold text-red-800">{error}</p>}<div className="mt-5 flex justify-end gap-3"><button type="button" onClick={close} className="rounded-lg border border-white/30 px-4 py-2.5 font-bold">Cancel</button><button type="button" disabled={busy || !result} onClick={() => void approve()} className="rounded-lg bg-[#d4af37] px-5 py-2.5 font-extrabold text-[#143d1a] disabled:opacity-50">{busy ? "Approving…" : "Approve Pricing"}</button></div></section>
  </div></section></div>;
}
...
function isPostConstructionInput(input: CalculatorInput): input is Extract<CalculatorInput, { calculatorType: "Post-Construction" }> { return "calculatorType" in input && input.calculatorType === "Post-Construction"; }
