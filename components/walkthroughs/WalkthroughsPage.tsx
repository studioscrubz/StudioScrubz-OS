...
export function WalkthroughsPage() {
  ...
  const canArchive = ["Master Admin", "Administrator", "Manager"].includes(profile?.role ?? "");
  ...
  const scheduled = useMemo(() => visible.filter((item) => (item.status === "New" || item.status === "Scheduled")&&Boolean(item.walkthrough_date&&item.walkthrough_time)).sort(compareWalkthroughSchedule), [visible]);
  ...
  return <>
    <header className="border-b border-[#143d1a]/10 pb-7 sm:pb-8"><p className="mb-3 text-[11px] font-extrabold uppercase tracking-[.2em] text-[#9a7a17]">Sales workspace</p><h1 className="text-3xl font-extrabold tracking-[-.04em] text-[#143d1a] sm:text-4xl">Sales Assessments</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-neutral-600 sm:text-base">Qualify estimate requests, document contact, choose an assessment method, and move completed property assessments into the existing pricing review and proposal flow.</p></header>
    ...
    {loading ? <Loading /> : <main className="mt-7 space-y-9">
      <section aria-labelledby="qualification-heading"><div className="flex items-end justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#9a7a17]">New · qualification / contacting</p><h2 id="qualification-heading" className="mt-1 text-xl font-extrabold text-[#143d1a]">Active Sales Qualification</h2></div><span className="text-sm font-bold text-neutral-500">{qualifying.length}</span></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{qualifying.map(item=><QualificationCard key={item.id} item={item} open={()=>setActive(item)}/>)}{qualifying.length===0&&<Empty text="No unscheduled assessments match these filters."/>}</div></section>
...
function QualificationCard({item,open}:{item:WalkthroughWithRelations;open:()=>void}){const method=item.measurements.assessmentMethod??"Qualification needed";return <article className="rounded-xl border border-[#143d1a]/10 bg-white p-4"><p className="text-[10px] font-extrabold uppercase text-[#9a7a17]">{method}</p><p className="mt-2 font-extrabold text-[#143d1a]">{displayClient(item.client)}</p><p className="text-sm text-neutral-500">{propertyLabel(item)}</p><p className="mt-1 text-xs text-neutral-500">{serviceLabel(item)}{item.estimate?.estimate_number?` · ${item.estimate.estimate_number}`:""}</p><button type="button" onClick={open} className="mt-4 rounded-lg bg-[#143d1a] px-4 py-2.5 text-xs font-bold text-white">Open Sales Assessment</button></article>}
function CompletedCard({ item, proposal, busy, canArchive, open, review, archive }: { item: WalkthroughWithRelations; proposal?: ProposalWithRelations; busy: boolean; canArchive: boolean; open: () => void; review: () => void; archive: () => void }) { return <article className="rounded-xl border border-[#143d1a]/10 bg-white p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className="rounded-full bg-[#edf4ec] px-2 py-1 text-[10px] font-extrabold text-[#143d1a]">COMPLETED — PRICING REVIEW</span><p className="mt-3 truncate font-extrabold text-[#143d1a]">{displayClient(item.client)}</p><p className="truncate text-sm text-neutral-500">{propertyLabel(item)}</p><p className="mt-1 text-xs font-semibold text-neutral-500">{serviceLabel(item)} · {item.walkthrough_date?friendlyDate(item.walkthrough_date):"Customer photo assessment"}</p></div><button type="button" onClick={open} className="rounded-lg border px-3 py-2 text-xs font-bold text-[#143d1a]">Open</button></div><div className="mt-4 flex flex-wrap gap-2">{proposal ? <Link href="/open-proposals" className="rounded-lg bg-[#143d1a] px-3 py-2 text-xs font-bold text-white">View Proposal</Link> : !item.pricing_review ? <button type="button" onClick={review} className="rounded-lg bg-[#d4af37] px-3 py-2 text-xs font-extrabold text-[#143d1a]">Review Pricing</button> : <Link href={`/proposals?walkthroughId=${encodeURIComponent(item.id)}`} className="rounded-lg bg-[#143d1a] px-3 py-2 text-xs font-bold text-white">Create Proposal</Link>}{canArchive && <button type="button" onClick={archive} disabled={busy} className="rounded-lg border px-3 py-2 text-xs font-bold text-neutral-500 disabled:opacity-50">Archive</button>}</div></article>; }
