...
export function WalkthroughsPage() {
  ...
  return <>
    ...
    {loading ? <Loading /> : <Pipeline walkthroughs={filtered} proposals={proposals} updatingId={updatingId} open={setActive} review={setReviewing} changeStatus={changeStatus} archive={archiveItem} />}
...
function Pipeline({ walkthroughs, proposals, updatingId, open, review, changeStatus, archive }: { walkthroughs: WalkthroughWithRelations[];proposals:ProposalWithRelations[]; updatingId: string | null; open: (item: WalkthroughWithRelations) => void;review:(item:WalkthroughWithRelations)=>void; changeStatus: (item: WalkthroughWithRelations, status: WalkthroughStatus) => Promise<void>; archive: (item: WalkthroughWithRelations) => Promise<void> }) { return <section aria-label="Walkthrough pipeline" className="mt-6 overflow-x-auto pb-4"><div className="grid min-w-[1040px] grid-cols-4 gap-4">{ACTIVE_WALKTHROUGH_STATUSES.map((column) => { const items = walkthroughs.filter((item) => item.status === column); return <div key={column} className="rounded-2xl border border-[#143d1a]/10 bg-[#eef1ed] p-3"><header className="mb-3 flex items-center justify-between px-1"><h2 className="text-xs font-extrabold tracking-[.1em] text-[#143d1a]">{column.toLocaleUpperCase()}</h2><span className="rounded-full bg-white px-2 py-0.5 text-xs font-bold text-neutral-500">{items.length}</span></header><div className="space-y-3">{items.map((item) => <WalkthroughCard key={item.id} item={item} proposal={proposals.find(p=>p.walkthrough_id===item.id&&!p.archived_at)} busy={updatingId === item.id} open={() => open(item)} review={()=>review(item)} change={(next) => void changeStatus(item, next)} archive={() => void archive(item)} />)}{items.length === 0 && <div className="rounded-xl border border-dashed border-[#143d1a]/15 px-3 py-8 text-center text-xs text-neutral-400">No walkthroughs</div>}</div></div>; })}</div></section>; }
function WalkthroughCard({ item, proposal, busy, open, review, change, archive }: { item: WalkthroughWithRelations;proposal?:ProposalWithRelations; busy: boolean; open: () => void;review:()=>void; change: (status: WalkthroughStatus) => void; archive: () => void }) { return <article className="rounded-xl border border-[#143d1a]/10 bg-white p-4 shadow-sm"><button type="button" onClick={open} className="w-full text-left"><p className="text-sm font-extrabold text-[#143d1a]">{displayClient(item.client)}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-neutral-500">{item.property?.address||"Deleted Property"}</p><p className="mt-2 text-xs font-bold text-[#143d1a]">{item.measurements.serviceType||item.estimate?.service_name||"Service not selected"}</p><div className="mt-3 flex items-center justify-between"><span className="rounded-full bg-[#edf4ec] px-2 py-1 text-[10px] font-bold text-[#143d1a]">{item.division}</span>{item.estimate && <span className="text-[10px] font-bold text-[#9a7a17]">{item.estimate.estimate_number}</span>}</div><dl className="mt-3 space-y-1 text-xs"><div className="flex justify-between"><dt className="text-neutral-400">Date</dt><dd className="font-semibold text-neutral-600">{item.walkthrough_date ? displayDate(item.walkthrough_date) : "—"}</dd></div><div className="flex justify-between"><dt className="text-neutral-400">Time</dt><dd className="font-semibold text-neutral-600">{item.walkthrough_time ? displayTime(item.walkthrough_time) : "—"}</dd></div><div className="flex justify-between"><dt className="text-neutral-400">Assigned</dt><dd className="max-w-28 truncate font-semibold text-neutral-600">{item.assigned_to || "Unassigned"}</dd></div></dl></button><div className="mt-4 border-t border-neutral-100 pt-3">{proposal?<Link href="/open-proposals" className="mb-2 block w-full rounded-lg bg-[#143d1a] px-3 py-2 text-center text-xs font-bold text-white">View Proposal</Link>:item.status==="Completed"&&!item.pricing_review?<button type="button" onClick={review} className="mb-2 w-full rounded-lg bg-[#d4af37] px-3 py-2 text-xs font-extrabold text-[#143d1a]">Review Pricing</button>:item.status==="Completed"&&item.pricing_review?<Link href={`/proposals?walkthroughId=${encodeURIComponent(item.id)}`} className="mb-2 block w-full rounded-lg bg-[#143d1a] px-3 py-2 text-center text-xs font-bold text-white">Create Proposal</Link>:null}<select aria-label="Change walkthrough status" value={item.status} onChange={(e) => change(e.target.value as WalkthroughStatus)} disabled={busy} className="h-9 w-full rounded-lg border border-neutral-200 px-2 text-xs font-bold text-neutral-600">{WALKTHROUGH_STATUSES.map((status) => <option key={status}>{status}</option>)}</select>{item.status !== "Archived" && <button type="button" onClick={archive} disabled={busy} className="mt-2 w-full text-center text-xs font-bold text-neutral-400 hover:text-red-700">Archive</button>}</div></article>; }
