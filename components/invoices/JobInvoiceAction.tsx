...
export function JobInvoiceAction({jobId,children}:{jobId:string;children:ReactNode}){
  ...
  function load(){return Promise.all([getInvoiceForJob(jobId),canCreateJobInvoice(jobId)]).then(([nextInvoice,allowed])=>{setInvoice(nextInvoice);setEligible(allowed);setError(null)}).catch(cause=>{console.error("Invoice relationship load failed",cause);setError("Unable to check Invoice status.")}).finally(()=>setLoading(false))}
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(()=>{void load()},[jobId]);
  useOperationalRealtime(["invoices","service_occurrences","service_agreements"],load);
  async function create(){setSaving(true);setError(null);try{const created=await createInvoiceFromJob(jobId);setInvoice(await getInvoiceForJob(jobId)??created)}catch(cause){console.error("Invoice creation failed",cause);setError(cause instanceof Error?cause.message:"Unable to create invoice.")}finally{setSaving(false)}}
