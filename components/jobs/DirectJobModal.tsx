...
export function DirectJobModal({ close, created }: { close: () => void; created: () => Promise<void> }) {
  ...
  const [serviceId, setServiceId] = useState(""), [addonNames, setAddonNames] = useState<string[]>([]), [addonSelections, setAddonSelections] = useState<CatalogAddonSnapshot[]>([]);
  ...
  const addons = useMemo(() => service && catalog ? getAvailableServiceAddons(catalog, service.id, property?.property_type) : [], [catalog, property?.property_type, service]);
  ...
  const addonPricing = useMemo(() => { try { return { rows: service ? calculateAddons(addonNames, addons, 0, addonSelections) : [], error: null }; } catch (cause) { return { rows: [], error: message(cause, "Enter valid Add-On quantities." ) }; } }, [addonNames, addonSelections, addons, service]);
  const addonTotal = addonPricing.rows.reduce((sum, row) => sum + row.amount, 0);
  const total = basePrice == null ? null : Math.round((basePrice + addonTotal) * 100) / 100;
  function selectClient(value: string) { setClientId(value); const matches = properties.filter((row) => row.client_id === value); setPropertyId(matches.length === 1 ? matches[0].id : ""); setServiceId(""); setAddonNames([]); setAddonSelections([]); }
  function selectProperty(value: string) { setPropertyId(value); setServiceId(""); setAddonNames([]); setAddonSelections([]); const found = properties.find((row) => row.id === value); setAccess(found?.access_instructions ?? ""); }
  async function submit() {
    ...
    if (addonPricing.error) return setError(addonPricing.error);
