...
export function DirectJobModal({ close, created }: { close: () => void; created: () => Promise<void> }) {
  ...
  async function submit() {
    ...
    if (total == null) return setError("The selected Service does not have usable catalog pricing for a direct Job.");
    setSaving(true); setError(null);
    try { await createDirectJob({ client_id: clientId, property_id: propertyId, service_id: serviceId, addon_ids: selectedAddonIds, scheduled_date: date || null, start_time: date && time ? time : null, estimated_duration: duration > 0 ? duration : null, assigned_crew_id: crewId || null, labor_hours: Math.max(0, laborHours), access_instructions: access.trim() || null, internal_notes: notes.trim() || null }); await created(); }
  ...
  <div className="space-y-6 p-6">{loading ? <div className="h-64 animate-pulse rounded-xl bg-neutral-100" /> : <>
    ...
    <Section title="Pricing"><div className="grid gap-4 sm:grid-cols-3"><Price label="Catalog / Base" value={basePrice}/><Price label="Add-Ons" value={addonTotal}/><Price label="Job Total" value={total} strong/></div><Field label="Planned Labor Hours"><input type="number" min="0" step="0.25" className={`${input} mt-2 max-w-xs`} value={laborHours || ""} onChange={(event) => setLaborHours(Number(event.target.value))} /></Field><p className="mt-2 text-xs text-neutral-500">Labor hours are operational planning data; catalog and add-on prices determine this Job total.</p></Section>
