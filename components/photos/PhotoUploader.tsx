...
export function PhotoUploader({ recordType, recordId, categories, readonly = false, canDelete = true, title }: { recordType: OperationalPhotoRecordType; recordId: string; categories: readonly OperationalPhotoCategory[]; readonly?: boolean; canDelete?: boolean; title?: string }) {
  ...
  async function upload() { if (!pending.length) return; if (!navigator.onLine) return setError("An internet connection is required to upload photos."); setBusy(true); setError(null); let next = photos; try { for (const queued of pending) { next = await uploadOperationalPhoto({ recordType, recordId, category: queued.category, caption: queued.caption, source: queued.source, file: queued.file, currentPhotos: next }); setPending((current) => current.filter((item) => item.id !== queued.id)); } await refresh(next); } catch (cause) { console.error("Photo upload failed", cause); try { await refresh(next); } catch (refreshCause) { console.error("Photo refresh after upload failure failed", refreshCause); } setError(message(cause, "Photo upload failed.")); } finally { setBusy(false); } }
  ...
  return <section className="mt-6 rounded-2xl border border-[#143d1a]/10 bg-white p-5"><h3 className="font-extrabold text-[#143d1a]">{title ?? "Photos"}</h3><p className="mt-1 text-sm text-neutral-500">{readonly ? "Operational photos are available read-only for this role." : "Take a new photo or choose existing photos, review them, then upload securely."}</p>{error && <p role="alert" className="mt-3 rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p>}{!readonly && <div className="mt-4"><PhotoCaptureQueue photos={pending} setPhotos={setPending} categories={categories} busy={busy} upload={() => void upload()} /></div>}<div className="mt-5"><PhotoGallery photos={display} readonly={readonly} canDelete={canDelete} remove={(photo) => void remove(photo)} saveCaption={(photo, value) => void saveCaption(photo, value)} /></div></section>;
}
