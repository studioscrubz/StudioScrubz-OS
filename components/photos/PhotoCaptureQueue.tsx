...
import { defaultOperationalPhotoCustomerVisibility, validateOperationalPhoto } from "@/lib/services/photoStorage";
import type { OperationalPhotoCategory, OperationalPhotoRecordType, PendingOperationalPhoto } from "@/types/photo";

export function PhotoCaptureQueue({ photos, setPhotos, categories, recordType, busy = false, upload, cameraLabel = "Take Photo", libraryLabel = "Choose From Library", uploadLabel }: { photos: PendingOperationalPhoto[]; setPhotos: (photos: PendingOperationalPhoto[]) => void; categories: readonly OperationalPhotoCategory[]; recordType: OperationalPhotoRecordType; busy?: boolean; upload?: () => void; cameraLabel?: string; libraryLabel?: string; uploadLabel?: string }) {
  ...
  function choose(files: FileList | null, source: "camera" | "library") {
    ...
    try {
      ...
      if (retakeId) {
        ...
        setRetakeId(null);
      } else setPhotos([...photos, ...selected.map((file) => ({ id: crypto.randomUUID(), file, category: defaultCategory, caption: "", source, customerVisible: defaultOperationalPhotoCustomerVisibility(recordType, defaultCategory) }))]);
      setError(null);
