...
export function BusinessSettingsPage(){
  ...
  async function save(){if(!value)return;if(!Number.isFinite(value.upkeep_adjustment_percent)||value.upkeep_adjustment_percent<0||value.upkeep_adjustment_percent>40){setError("Upkeep Adjustment must be between 0% and 40%.");return}const phone=value.post_construction_payment_recipient_phone.replace(/\D/g,"");if(!Number.isFinite(value.post_construction_deposit_percent)||value.post_construction_deposit_percent<=0||value.post_construction_deposit_percent>100){setError("Post-Construction deposit percentage must be greater than 0% and no more than 100%.");return}if(value.post_construction_payment_method!=="Zelle"||value.post_construction_payment_recipient_name.trim().length<2||phone.length<10||phone.length>15||!value.post_construction_payment_memo_template.trim()||value.post_construction_payment_memo_template.trim().length>300){setError("Enter valid Post-Construction Zelle recipient, phone, and memo settings.");return}setSaving(true);setError(null);try{const{id:_,created_at:__,updated_at:___,...input}=value;setValue(await updateBusinessSettings({...input,post_construction_payment_recipient_phone:phone} as BusinessSettingsUpdate));setNotice("Business settings saved.")}catch(x){setError(msg(x))}finally{setSaving(false)}}
  return <><header className="border-b pb-7"><h1 className="text-3xl font-extrabold text-[#143d1a]">Business Settings</h1><p className="mt-3 text-neutral-600">Manage StudioScrubz business information and default document settings.</p></header>{error&&<Alert text={error}/>} {notice&&<Alert text={notice} good/>}
    ...
    <Section title="Post-Construction Deposit"><Grid><NumberField label="Required deposit %" value={value.post_construction_deposit_percent} min="0.01" max="100" set={x=>set("post_construction_deposit_percent",x)}/><label className="text-sm font-bold">Payment Method<select className={input} value={value.post_construction_payment_method} onChange={event=>set("post_construction_payment_method",event.target.value as "Zelle")}><option value="Zelle">Zelle</option></select></label><Field label="Recipient Name" value={value.post_construction_payment_recipient_name} set={x=>set("post_construction_payment_recipient_name",x)}/><Field label="Recipient Phone" value={formatPhone(value.post_construction_payment_recipient_phone)} set={x=>set("post_construction_payment_recipient_phone",x)}/><Area label="Memo Template" value={value.post_construction_payment_memo_template} set={x=>set("post_construction_payment_memo_template",x)}/></Grid><p className="mt-3 text-sm text-neutral-600">Use {"{{proposal_number}}"} and {"{{client_name}}"} to identify each payment. Changes affect only new or deliberately refreshed unpaid instructions.</p></Section>
...
function NumberField({label,value,set,min="0",max}:{label:string;value:number;set:(x:number)=>void;min?:string;max?:string}){return <label className="text-sm font-bold">{label}<input type="number" min={min} max={max} step="0.01" className={input} value={value} onChange={e=>set(globalThis.Number(e.target.value))}/></label>}
...
function formatPhone(value:string){const digits=value.replace(/\D/g,"");return digits.length===10?`(${digits.slice(0,3)}) ${digits.slice(3,6)}-${digits.slice(6)}`:value}
