...
export function BusinessSettingsPage(){
  ...
  async function save(){if(!value)return;if(!Number.isFinite(value.upkeep_adjustment_percent)||value.upkeep_adjustment_percent<0||value.upkeep_adjustment_percent>40){setError("Upkeep Adjustment must be between 0% and 40%.");return}setSaving(true);setError(null);try{const{id:_,created_at:__,updated_at:___,...input}=value;setValue(await updateBusinessSettings(input as BusinessSettingsUpdate));setNotice("Business settings saved.")}catch(x){setError(msg(x))}finally{setSaving(false)}}
