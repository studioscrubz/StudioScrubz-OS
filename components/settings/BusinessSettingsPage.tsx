...
function Area({label,value,set}:{label:string;value:string;set:(x:string)=>void}){return <label className="text-sm font-bold">{label}<textarea className={`${input} h-24 py-2`} value={value} onChange={e=>set(e.target.value)}/></label>}
