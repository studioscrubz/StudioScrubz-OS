...
function CatalogForm({ kind, value, services, close, saved }: { kind: Tab; value?: RecordRow; services: CatalogService[]; close: () => void; saved: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-[100] grid place-items-center bg-black/60 p-4"><section className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white p-6">
    ...
    {kind === "Add-Ons" && <AddonPricingFields config={pricingConfig(data.pricing_config)} set={(key, next) => setData((current) => ({ ...current, pricing_config: { ...pricingConfig(current.pricing_config), [key]: next } }))} />}
...
function AddonPricingFields({ config, set }: { config: PricingConfig; set: (key: string, value: string | number) => void }) { const perUnit=config.pricing_type==="Per Unit";return <section className="mt-6 rounded-xl border border-[#143d1a]/10 bg-neutral-50 p-4"><h3 className="font-extrabold text-[#143d1a]">Add-On Pricing</h3><div className="mt-4 grid gap-4 sm:grid-cols-3"><label className="text-sm font-bold">Pricing Type<select className={inputClass} value={String(config.pricing_type??"Flat Price")} onChange={event=>set("pricing_type",event.target.value)}><option>Flat Price</option><option>Per Unit</option></select></label>{perUnit&&<><label className="text-sm font-bold">Unit Name<input className={inputClass} value={String(config.unit_name??"")} onChange={event=>set("unit_name",event.target.value)}/></label><label className="text-sm font-bold">Price Per Unit<input className={inputClass} type="number" min="0" step="0.01" value={Number(config.unit_price??0)} onChange={event=>set("unit_price",Number(event.target.value))}/></label></>}</div></section>}
...
function formFields(kind: Tab, services: CatalogService[]): FormField[] {
  ...
  return [{ key: "rule_name", label: "Rule Name" }, { key: "service_id", label: "Service", options: [{ label: "All Services", value: "" }, ...serviceOptions.slice(1)] }, { key: "frequency", label: "Frequency", options: options(["Daily", "Weekly", "Biweekly", "Twice Monthly", "Monthly", "One-Time", "Every 4 Weeks", "Multiple Days Per Week"]).map((option) => ({ ...option, label: serviceFrequencyLabel(option.value) })) }, { key: "adjustment_type", label: "Adjustment Type", options: options(RECURRING_ADJUSTMENT_TYPES) }, { key: "adjustment_value", label: "Adjustment Value", type: "number" }, { key: "is_active", label: "Active", type: "checkbox" }];
}
