...
export function ArchivesPage() {
  ...
  async function remove(record: ArchivedRecord) { setBusy(record.id); setError(null); try { await permanentlyDeleteArchivedRecord(record); } catch (cause) { setError(message(cause)); } finally { setConfirming(null); setBusy(null); } }
  return <>
    ...
    {loading ? <div className="mt-6 h-64 animate-pulse rounded-2xl bg-neutral-200" /> : visible.length ? <div className="mt-6 overflow-x-auto rounded-2xl border bg-white"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-[#edf1eb] text-xs uppercase text-[#143d1a]"><tr>{["Record Type", "Record", "Related", "Archived Date", "Status", "Actions"].map((label) => <th className="p-4" key={label}>{label}</th>)}</tr></thead><tbody>{visible.map((record) => <tr className="border-t" key={`${record.type}-${record.id}`}><td className="p-4 font-bold">{record.type}</td><td className="p-4 font-extrabold text-[#143d1a]">{record.label}</td><td className="p-4 text-neutral-600">{record.relatedName ?? "—"}</td><td className="p-4">{date(record.archivedAt)}</td><td className="p-4">{record.status}</td><td className="p-4"><div className="flex flex-wrap gap-2"><Link href={record.href} className={secondary}>View</Link><button disabled={busy === record.id} className={secondary} onClick={() => void restore(record)}>Restore</button><button disabled={busy === record.id} className={danger} onClick={() => setConfirming(record)}>Delete Permanently</button></div></td></tr>)}</tbody></table></div> : <div className="mt-6 rounded-2xl border border-dashed bg-white p-12 text-center text-neutral-500">No archived records in this category.</div>}
